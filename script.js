...
function toDoPage(){
  ...
  function taskListUpdater(){
    ...
    tasklist.forEach((task,idx) => {
      sum+= `<div id=${idx} class="taskItem min-h-[80px] w-[100%] p-4 border-[1px] ${task.important?'bg-green-200':''} border-black flex gap-[12.5%] justify-between items-center bg-[rgba(255,255,255,0.1)] hover:bg-[rgba(255,255,255,0.2)] ${task.important?'hover:bg-green-100':''} hover:text-black transition text-black/60 backdrop-blur-xs font-[gs]">
                  <div class="left flex flex-col justify-between items-start w-[40%] overflow-hidden">
                  <span class="taskname text-[20px] capitalize bg-[rgba(0,0,0,0.05)] rounded-[5px] px-3">${(idx+1) +"'  " + task.name}</span>
  ...
  const imp = todoTitle.value.trim()[0]=='*';
  
  todoTitle.value='';
  todoDets.value='';
  tasklist.push({name:taskname, details:taskdets, important: imp, time:curr.toLocaleTimeString("en-US"), day:curr.toLocaleDateString( "en-US", { weekday: "long" })})
